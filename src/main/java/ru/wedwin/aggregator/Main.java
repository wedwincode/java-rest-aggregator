...
import ru.wedwin.aggregator.adapter.out.output.ConsoleWriter;
import ru.wedwin.aggregator.adapter.out.output.CsvFileWriter;
...
//                List.of(new CsvFileWriter(), new JsonFileWriter())
//        );
        AggregationUseCase useCase = new AggregationUseCase(
                new CliApp(args),
                new OkHttpExecutor(),
                List.of(new NewsApiClient()),
